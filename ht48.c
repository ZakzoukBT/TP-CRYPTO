...
* Input m: the message to be hashed, whose length is required to be an integral number of bytes
* Input h: placeholder for the 48-bit resulting tag, to be stored as an array of 6 bytes. Must have been allocated by the caller.
...
void smht48(const uint8_t k[static 6], uint64_t blen, const uint8_t m[blen], uint8_t h[static 6]){
    uint64_t size = blen + 6;
    uint8_t concat_message[size];
    for (int i=0; i <blen; i++)
    {
        concat_message[i] = m[i];
    } 
    for (int i = 0; i < 6; i++){
        int tmp = blen + i;
        concat_message[tmp] = k[i];
    }
